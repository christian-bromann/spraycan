function Spraycan(video,dataCanvas,drawCanvas) {
    ...
    window.addEventListener('cleanUp', function(){ this.cleanUp(); }.bind(this), false);
...
Spraycan.prototype.ptr2XY = function(i) {
    ...
    return [
        // x
        i % this.drawCanvas.width,
        // y
        Math.floor(i/this.drawCanvas.width)
    ];
};
...
Spraycan.prototype.xy2Ptr = function(x,y) {
    x *= 4;
    y *= 4;
    return (y * this.drawCanvas.width) + x;
};
...
Spraycan.prototype.findLightPoint = function(data) {
    ...
    if(this.foundX && this.foundY) {
        ...
        this.drawStrokeHelper(this.foundX, this.foundY,colors[Math.floor(Math.random()*7)],50);

        var start = this.xy2Ptr(this.foundX - 50,this.foundY - 50),
            end   = this.xy2Ptr(this.foundX + 50,this.foundY + 50);

        nextbreak = this.xy2Ptr(this.foundX + 50 > this.drawCanvas.width ? this.drawCanvas.width : this.foundX + 50,this.foundY - 50);
        i = start;

        // console.log('go from ', [this.foundX,this.foundY],' to ', this.ptr2XY(nextbreak), ' ---- ',i,nextbreak);
        while(i < end) {

            // this.drawContext.fillRect(this.ptr2XY(i)[0],this.ptr2XY(i)[1],1,1);
            if(i > nextbreak) {

                nextbreak = this.ptr2XY(nextbreak);
                // console.log('reached ', this.ptr2XY(i));
                // console.log('new point ', [nextbreak[0]-100,nextbreak[1]+1]);
                // console.log('new break ', [nextbreak[0],nextbreak[1]+1]);

                // this.drawStrokeHelper([nextbreak[0]-100,nextbreak[1]+1],'rgb(255,255,255)',2);
                // this.drawStrokeHelper([nextbreak[0]-100,nextbreak[1]+1],'rgb(255,255,255)',4);
                // this.drawStrokeHelper([nextbreak[0],nextbreak[1]+1],'rgb(255,255,255)',2);
                // this.drawStrokeHelper([nextbreak[0],nextbreak[1]+1],'rgb(255,255,255)',4);

                // console.log('new i ',nextbreak[0]-100,nextbreak[1]+1);
                i = this.xy2Ptr(nextbreak[0]-100,nextbreak[1]+1);
                nextbreak = this.xy2Ptr(nextbreak[0],nextbreak[1]+1);
            ...
            if(r > 200) {
                ...
                // this.drawContext.fillRect(x,y,2,2);
        ...
        // this.drawStrokeHelper(x,y,'rgb(33,202,102)');
        // console.log('reach final point ',this.ptr2XY(i), ' (end was ',this.ptr2XY(end));
        // undefined.length;
...
Spraycan.prototype.spray = function(x,y,d,distance,angle) {
    ...
    //     this.timestamp = currentTime;
        ...
        // this.drawStrokeHelper(x-1,y-1,'rgb(254,0,0)',2);
        this.drawStrokeHelper(x-2,y-2,'rgb(254,0,0)',4);
