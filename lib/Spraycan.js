...
Spraycan.prototype.draw = function() {
    ...
    if(this.foundX && this.foundY) {
        idata = this.dataContext.getImageData( this.minLeft() , this.minTop() , 100 , 100 );
        // this.drawStrokeHelper(this.foundX, this.foundY,'rgb(255,0,0)',50);
    } else {
        idata = this.dataContext.getImageData(0,0,this.dataCanvas.width,this.dataCanvas.height);
    }
...
Spraycan.prototype.spray = function(x,y,distance,angle) {
    ...
    if (this.isClear() || (currentTime - this.timestamp > 500)) {
        ...
        for ( var z=0; (z<=distance || z===0); z++ ) {
            ...
            this.drawContext.drawImage(this.brush, _x, _y, 5, 5);
        }
        this.lastPoint = [x,y];
        this.timestamp = currentTime;
