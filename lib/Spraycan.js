function Spraycan(video,dataCanvas,drawCanvas) {
    ...
    // increase/decrease brush size depending on gyroscope values
    this.socket.on('gyroscope',function(gyro) {
        // this.brush.style.width = Math.abs(gyro.y - 180) + 'px';
        // this.brush.style.webkitTransform = 'rotate(' + gyro.z + 'deg)';
        this.gyro = gyro;
    }.bind(this));
    
    // set brush color depending on cap resistor
    this.socket.on('resistance', function(resistance) {
        if(Math.abs(resistance) - 1800 < this.e) {
            this.brush = this.brushGreen;
        } else if(Math.abs(resistance - 2000) < this.e) {
